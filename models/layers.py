...
class TMA(nn.Module):
    ...
    def __init__(self, dim, topk, num_frames_q, num_frames_kv, window_size, num_heads,
    ...
    def forward(self, q, kv=None, mask=None):
        ...
        mask1 = torch.zeros(B_, self.num_heads, N1, N2, device=q.device, requires_grad=False)

        attn = (q @ k.transpose(-2, -1))  # B_, nH, N1, N2
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            N1, N2, -1)  # D1*Wh*Ww, D2*Wh*Ww, nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()
        # mask1 = create_mask_from_attention_matrix(attn, threshold=0.2)
        # mask1 = torch.tensor(mask1)
        # mask1 = mask1.to(atqtn.device)
     
        # for idx in above_threshold_indices:
        #     mask1[idx[0], idx[1], idx[2], idx[3]] = 1
        # index1 = torch.nonzero(attn_sigmoid > 0.2, as_tuple=False)

        
        index = torch.topk(attn, k=int(N2 / 8), dim=-1, largest=True)[1]
        ...
        # if self.topk == 0:
        #
        #     index = torch.topk(attn, k=int(N2 / 5), dim=-1, largest=True)[1]
        #     mask1.scatter_(-1, index, 1.)
        #     attn1 = torch.where(mask1 > 0, attn, torch.full_like(attn, float('-inf')))
        #     attn1 = attn1 + relative_position_bias.unsqueeze(0)
        # if self.topk == 1:
        #
        #     index = torch.topk(attn, k=int(N2 / 6), dim=-1, largest=True)[1]
        #     mask1.scatter_(-1, index, 1.)
        #     attn1 = torch.where(mask1 > 0, attn, torch.full_like(attn, float('-inf')))
        #     attn1 = attn1 + relative_position_bias.unsqueeze(0)
        # if self.topk == 2:
        #
        #     index = torch.topk(attn, k=int(N2 / 7), dim=-1, largest=True)[1]
        #     mask1.scatter_(-1, index, 1.)
        #     attn1 = torch.where(mask1 > 0, attn, torch.full_like(attn, float('-inf')))
        #     attn1 = attn1 + relative_position_bias.unsqueeze(0)
        # if self.topk == 3:
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn1 = attn1.view(B_ // nW, nW, self.num_heads, N1, N2) + mask.unsqueeze(1).unsqueeze(
                0)  # B, nW, nH, D1*Wh*Ww, D2*Wh*Ww
            attn1 = attn1.view(-1, self.num_heads, N1, N2)
        ...
        x = (attn1 @ v).transpose(1, 2).reshape(B_, N1, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn1
...
class VSTSREncoderTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, topk, num_heads, num_frames=4, window_size=(8, 8),
        ...
        self.attn = TMA(
            dim, topk, num_frames_q=self.num_frames, num_frames_kv=self.num_frames,
...
class VSTSRDecoderTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, topk, num_heads, num_frames=4, window_size=(8, 8),
        ...
        self.norm1 = norm_layer(dim)
        self.attn1 = TMA(
            dim, topk, num_frames_q=self.num_out_frames,
    ...
    def forward(self, x, attn_kv, mask_matrix_q, mask_matrix_qkv):
        ...
        x_windows = x_windows.view(-1, D1 * window_size[0] * window_size[1], C)  # nW*B, D1*window_size*window_size, C
        attn_kv_windows = attn_kv_windows.view(-1, D2 * window_size[0] * window_size[1],
                                               C)  # nW*B, D2*window_size*window_size, C

        # W-MSA/SW-MSA
        attn_windows = self.attn2(x_windows, attn_kv_windows, mask=attn_mask_qkv)[
            0]  # nW*B, D1*window_size*window_size, C
...
class EncoderLayer(nn.Module):
    def __init__(self, dim, topk, depth, num_heads, num_frames, window_size=(8, 8),
        ...
        self.blocks = nn.ModuleList([
            VSTSREncoderTransformerBlock(dim=dim, topk=topk, num_heads=num_heads,
...
class DecoderLayer(nn.Module):
    def __init__(self, dim, topk, depth, num_heads, num_frames, window_size=(8, 8),
        ...
        # Build blocks
        self.blocks = nn.ModuleList([
            VSTSRDecoderTransformerBlock(dim=dim, topk=topk, num_heads=num_heads,
                                         num_frames=num_frames, window_size=window_size,
                                         shift_size=(0, 0) if (i % 2 == 0) else self.shift_size,
                                         mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                         drop=drop, attn_drop=attn_drop,
...
class TemporalAttention(nn.Module):
    ...
    def __init__(self, d_model,h_model, kernel_size=21, attn_shortcut=True):
        super().__init__()

        self.proj_1 = nn.Conv2d(d_model, h_model, 1)         # 1x1 conv
        self.activation = nn.GELU()                          # GELU
        self.spatial_gating_unit = TemporalAttentionModule(h_model, kernel_size)
        self.proj_2 = nn.Conv2d(h_model, d_model, 1)         # 1x1 conv
...
class TemporalAttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("1",x.shape)
        attn = self.conv0(x)
        # print("2",attn.shape)# depth-wise conv
        attn = self.conv_spatial(attn)
        # print("3",attn.shape)# depth-wise dilation convolution
        f_x = self.conv1(attn)
        # print("4",f_x.shape)# 1x1 conv
        # append a se operation
        b, c, _, _ = x.size()
        se_atten = self.avg_pool(x).view(b, c)
        # print("5",se_atten.shape)
        se_atten = self.fc(se_atten).view(b, c, 1, 1)
        # print((se_atten * f_x).shape)
        # print("-------------")
        # print(u.shape)
        # print("6",se_atten.shape)
        return se_atten * f_x * u

class TFRB_encoder(nn.Module):
    def __init__(self, d_model=384,h_model=256, kernel_size=21, attn_shortcut=True):
        super().__init__()
        enc_layers = []
        for i in range(4):
            enc_layers.append(TemporalAttention(d_model, h_model,kernel_size, attn_shortcut))
        self.enc = nn.Sequential(*enc_layers)
        # self.proj1 = nn.Conv2d(384, 256, kernel_size=(1, 1), stride=(1, 1))
        # self.proj2 = nn.Conv2d(256, 384, kernel_size=(1, 1), stride=(1, 1))
    def forward(self, x):
        B, T, C, H, W = x.shape
        # print(x.shape)
...
class TFRB_decoder(nn.Module):
    def __init__(self, d_model=672,h_model=256, kernel_size=21, attn_shortcut=True):
        super().__init__()
        enc_layers = []
        for i in range(3):
            enc_layers.append(TemporalAttention(d_model,h_model, kernel_size, attn_shortcut))
        ...
        # self.proj1 = nn.Conv2d(672, 256, kernel_size=(1, 1), stride=(1, 1))
        # self.proj2 = nn.Conv2d(256, 672, kernel_size=(1, 1), stride=(1, 1))
    def forward(self, x):
        B, T, C, H, W = x.shape
        # print(x.shape)
        x = x.reshape(B, T*C, H, W)
...
class PVT2FFN(nn.Module):
    def __init__(self, in_features, hidden_features):
        ...
        # self.dwconv5x5 = nn.Conv2d(hidden_features, hidden_features, 5, 1, 2, bias=True, groups=hidden_features)
        self.act = nn.GELU()
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        _, _, _, _, C1 = x.size()
        x = x.permute(0, 1, 4, 2, 3).reshape(-1, C1, H, W)
        x = self.dwconv(x)
        x = x.view(B, D, C1, H, W).permute(0, 1, 3, 4, 2)
        x = self.act(x)
        x = self.fc2(x)
